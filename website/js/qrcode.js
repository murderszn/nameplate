 ...
 * Supports Byte encoding, Error Correction Levels L, M, Q, H,
 * and outputs crisp SVG or Canvas elements.
...
(function (global) {
  ...
  var QRUtil = {
    ...
    getLostPoint: function (qrCode) {
      ...
      var lostPoint = 0;
      var row, col, count, sameCount, r, c;
  ...
  function createQRSvg(text, options) {
    options = options || {};
    var ec = QRErrorCorrectLevel[options.ecLevel || 'H'];
    var qr = new QRCode(0, ec !== undefined ? ec : QRErrorCorrectLevel.H);
    qr.addData(text);
    qr.make();
    ...
    var dark = options.dark || '#0E1620';
    var light = options.light || 'transparent';
    ...
    return [
      ...
      light !== 'transparent' ? '<rect width="' + viewBoxSize + '" height="' + viewBoxSize + '" fill="' + light + '"/>' : '',
      '<path d="' + pathData.join('') + '" fill="' + dark + '"/>',
      '</svg>'
    ].join('');
  }
